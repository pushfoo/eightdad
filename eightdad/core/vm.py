...
class Breakpoint:
    """
    Psuedo-immutable breakpoint.

    Implemented as a class because a name is autogenerated if none is
    passed at creation. Doing that with a namedtuple would be ugly.

    """
    def __init__(self, address: int, name: str = None, enabled: bool = True):
        """
        If no name is given, a default will be created using the address.

        :param address: what address in memory the breakpoint will be at.
        :param name: what name, if any, the breakpoint has.
        :param enabled: if the VM should halt on it.
        """
        self._address: int = address
        self.enabled: bool = enabled
        self._name: str = name or f"breakpoint-0x{upper_hex(address)}"
